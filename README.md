...
매 실행 끝에 부위 배분을 스스로 검사한다.
날짜·부위별로 포장이 받은 양과 파쇄가 내보낸 양(세척 후)을 비교해
5% 넘게 어긋나면 목록으로 남긴다. 4~9월 실적으로는 0건이므로 뜨면 실제 신호다.

부위 판정이 한쪽으로 몰리거나 새는 거의 모든 경우가 여기서 걸린다.
같이 출력되는 '수동입력 불일치'와 '부위 미해결' 목록도 함께 확인한다.

알려진 한계: `wagonDist` 는 대차번호를 키로 쓰는 map 이라
한 포장이 같은 번호를 하루에 두 번 받으면 현장 입력이 합산돼 저장된다.
그 경우 두 부위에 걸친 양이 한 부위로 몰리며, 위 점검에서 드러난다.
근본 해결은 입력을 `{번호, kg}` 목록으로 바꾸는 것이다.
